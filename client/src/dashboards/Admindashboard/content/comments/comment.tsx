...
export default function AdminComment() {
  ...
  // Handle creating a comment (admin)
  ...
  // Handle deleting a comment (admin)
  ...
  if (isLoading)
    return (
      <p data-test="loading-comments" className="text-center text-gray-500">
        Loading comments...
      </p>
    );
    ...
    <div className="mt-6 p-4 bg-white rounded-xl shadow-md border border-gray-200 max-w-4xl mx-auto">
          ...
          comments.map((c) => {
            ...
            return (
              ...
              >
                ...
                {/* Delete button (admin can delete all) */}
                {user.role === "admin" && (
                  <button
      ...
      <div className="mt-4 flex flex-col md:flex-row items-center gap-2">
        <input
          ...
          placeholder="Bug ID"
          className="input input-bordered w-full md:w-32 bg-white text-black border-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-150"
        />
        ...
        <button
          ...
          disabled={creating || !content.trim() || bugid === ""}
